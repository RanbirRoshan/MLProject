...
def GetCNNModel(X_train):
    ...
    model.add(Conv2D(filters=4, kernel_regularizer=regularizers.l2(0.01), strides=(1,1), kernel_size=(2,4)))
    model.add(Activation('relu'))
    model.add(MaxPool2D((2,2)))
    model.add(Conv2D(filters=4, kernel_regularizer=regularizers.l2(0.01), strides=(1,1), kernel_size=(3,2)))
    model.add(Activation('relu'))
    model.add(MaxPool2D((2,2)))
